...
export async function generateFakerConfig(allExtractedSchemas) {
  ...
  try {
    ...
    // Write the Exported RuleBook to a file
...
// // 1. Initialize the official OpenRouter SDK
...
// // // Helper to strip markdown formatting if the free model disobeys instructions
...
//   try {
//     // 2. Use the native chat.send method
...
//     const outputPath = path.join(outputDir, `RuleBook.json`);
//     // Write the Exported RuleBook to a file
//     fs.writeFileSync(outputPath, JSON.stringify(JSON.parse(rawText).rulebook, null, 2));

//     return JSON.parse(rawText).rulebook;
